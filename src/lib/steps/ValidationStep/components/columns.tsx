      ...
      editor: ({ row, onRowChange, onClose }) => {
        ...
        switch (column.fieldType.type) {
          case "select":
            component = (
              <TableSelect
                value={column.fieldType.options.find((option: { value: string }) => option.value === (row[column.key as T] as string))}
                onChange={(value) => {
          ...
          case "select":
            component = (
              <Box minWidth="100%" minHeight="100%" overflow="hidden" textOverflow="ellipsis">
                {column.fieldType.options.find((option: { value: any }) => option.value === row[column.key as T])?.label || null}
              </Box>
            )
            break
          default:
